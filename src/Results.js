...
function Results (props) {
  ...
  for (let i = 0; i < igc.fixes.length && i / igc.fixes.length < progress; i++) {
    ...
    if (distance < turnpoint.radius) {
      ...
      const durationSec = (moment(`1970-01-01 ${fix.time}Z`) - moment(`1970-01-01 ${turnpointTimes[0]}Z`)) / 1000
      ...
      turnpoints.push(
        ...
        >
          <small>{turnpoint.waypoint.description}{fix.time}<br/>{Math.floor(durationSec / 60)}:{durationSec % 60}
          </small>
        </div>
      )
      let center = [turnpoint.waypoint.lat, turnpoint.waypoint.lon]
      circles.push(<Circle center={center} pathOptions={{ fillColor: 'blue', color: 'transparent', opacity: 0.25 }}
                           radius={turnpoint.radius} key={'turnpoint-circle-' + turnpointIndex}/>)
      turnpointIndex++
    }
  }
  ...
  for (let i = task.turnpoints.length - 1; i >= turnpointIndex; i--) {
    ...
    turnpoints.splice(turnpointIndex, 0, <div key={'turnpoint-' + i}
                                              style={{ ...listStyle, borderColor: color, zIndex: i }}>
      <small>{turnpoint.waypoint.description}</small>
    </div>)
    let center = [turnpoint.waypoint.lat, turnpoint.waypoint.lon]

    circles.push(<Circle center={center} pathOptions={{ color: color, fillColor: fillColor, opacity: opacity }}
                         radius={turnpoint.radius} key={'turnpoint-circle-' + i}/>)
  }
  ...
  return <div style={containerStyle}>
    ...
    <div className="rightHandContainer"
         style={{ alignSelf: 'stretch', width: '100%', display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div className="flightProgress" style={{ height: '80px', backgroundColor: 'blue', display: 'flex' }}>
        <input type="range" min="1" max="100" class="slider" id="myRange" style={{ flexGrow: 1 }}
